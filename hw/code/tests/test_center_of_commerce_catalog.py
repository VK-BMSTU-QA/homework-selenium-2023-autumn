...
class TestCenterOfCommerceCatalog(BaseCase):
    ...
    def test_catalog_creation_works(self, tab, second_field, center_of_commerce_page: CenterOfCommercePage, cookies_and_local_storage):
        center_of_commerce_page.go_to_create_catalog(tab, second_field, TIMEOUT)
    ...
    def test_catalog_category_switch_works(self, from_what, to, center_of_commerce_page: CenterOfCommercePage, cookies_and_local_storage):
        ...
        assert center_of_commerce_page.find_catalog_title(to, TIMEOUT) is not None
    ...
    def test_catalog_category_tabs_redirecting(self, catalog, tab, tab_id, center_of_commerce_page: CenterOfCommercePage, cookies_and_local_storage):
        ...
        assert center_of_commerce_page.check_catalog_tab_switched(tab, TIMEOUT) is not None
    ...
    @pytest.mark.parametrize("catalog, product_id, title", [("Товары – fff", "8005304", "Тени для век Glam Look матовые, палетка"), ("Товары – fff", "8008361", "Лак для ногтей GEL SHINE перламутровый")])
    def test_catalog_products_search(self, catalog, product_id, title, center_of_commerce_page: CenterOfCommercePage, cookies_and_local_storage):
        center_of_commerce_page.go_to_catalog(catalog, TIMEOUT)
        center_of_commerce_page.search_product(product_id, TIMEOUT)

        assert center_of_commerce_page.find_product_by_title(title, TIMEOUT) is not None
    ...
    def test_catalog_products_table_settings_widget(self, catalog, center_of_commerce_page: CenterOfCommercePage, cookies_and_local_storage):
        ...
        assert center_of_commerce_page.find_table_settings_title(TIMEOUT) is not None
    ...
    def test_catalog_product_widget(self, catalog, product, product_id, center_of_commerce_page: CenterOfCommercePage, cookies_and_local_storage):
        center_of_commerce_page.go_to_catalog(catalog, TIMEOUT)
        center_of_commerce_page.go_to_catalog_product(product_id, product, TIMEOUT)

        assert center_of_commerce_page.find_h2_with_text(product, TIMEOUT) is not None
    ...
    def test_catalog_product_sort(self, catalog, product_id, center_of_commerce_page: CenterOfCommercePage, cookies_and_local_storage):
        ...
        assert center_of_commerce_page.find_product_by_id(product_id, TIMEOUT) is not None
    ...
    def test_catalog_warning_button(self, catalog, redirected_tab, center_of_commerce_page: CenterOfCommercePage, cookies_and_local_storage):
        ...
        assert center_of_commerce_page.check_catalog_tab_switched(redirected_tab, TIMEOUT)
